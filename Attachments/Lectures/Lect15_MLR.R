...
#           Author: Cody Chiuzan; Date: Nov 13, 2017            #
################################################################
...
# Simple linear regression with factor predictor REGION (1:NE, 2:NC, 3:S, 4:W)
data_hosp %>% lm(LOS~factor(REGION)+BEDS, data=.) %>% summary


# Simple linear regression with factor predictor REGION (1:NE, 2:NC, 3:S, 4:W)
# NO INTERCEPT
data_hosp %>% lm(LOS~0+factor(REGION)+BEDS, data=.) %>% summary
...
lines(gen_risk, yhat0, lwd=2, col=3)
